...
# Load the sf package
pacman::p_load(sf, janitor, raster)
...
forests <- read_clean_sf("../data/forests.shp") 

# Read in the parks shapefile
parks <- read_clean_sf("../data/parks.shp")

# Read in the playgrounds shapefile
playgrounds <- read_clean_sf("../data/playgrounds4326.shp")

# Read in the shelters json
shelters <- read_clean_sf("../data/shelters.json")

# View the first few features of all layers
all_stuff <- list(forests, parks, playgrounds, shelters)
sapply(all_stuff, head)
```

## Questions: 

1. How many features does each layer contain and what kind of geometry are they?
`forests` and `parks` seem to be multipolygons and the other two are points. All objects have 6 features, but the number of fields differ and are 15, 10, 5, and 5 for `forests`, `parks`, `playgrounds`, and `shelters` respectively. 
2. What is the CRS value in these objects?
ETRS89 / UTM zone 32N for all of them it seems
...
# Use filter() to limit to over5ha parks (50k m2)
over5ha <- parks %>% 
...
# Area and histogram
forest_area <- st_area(forests)
hist(forest_area)
# Plot stuff
forests %>% 
  filter(unclass(st_area(.)) > 200000) %>%
  st_geometry() %>% 
  plot

raster::plot(forests['navn'], legend=F)
```
...
```{r}
forest_geom <- st_geometry(forests)
```
...
```{r}
plot(forests$geometry, col='green');plot(parks$geometry, col='pink', add=T)
```
...
orthophoto <- brick('../data/Aarhus_1m.TIF')
raster_objs <- list(elevation, orthophoto)

lapply(raster_objs, class)
lapply(raster_objs, nlayers)
```
...
elevation
```
the dimensions seems to be **396 rows and 876 columns**

2. What is the resolution of `aarhus` orthophoto? How many layers does it contain and what do they represent?
```{r}
orthophoto
```
It seems to have a resolution of 1 meter(?) with a depth of five bands, which may be different frequencies?
...
It seems that the elevation data is in degrees, while the Aarhus city is in some different CRS that is more fine. This is a matter of them using different projections. 
...
plotRGB(orthophoto, stretch='linear')
```
* Try to plot the two images together (on top of one another). Can you do it?
```{r}
#plot(elevation);plotRGB(orthophoto, add=TRUE)
```
It crashed my R session :((
