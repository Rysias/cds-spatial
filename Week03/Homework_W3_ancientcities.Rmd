...
from the `st_crs` output we can see that the unit is meters (from the `LENGTHUNIT` attribute)
...
plot(cities_sf3035[1:10, "geometry"], col = "yellow")
plot(cities_5km[1:10, 'geometry'], pch=20, cex = 0.1, add = TRUE)
...
leaflet() %>% 
  addTiles() %>% 
  addMarkers(lng=i$longitude,lat=i$latitude,clusterOptions=markerClusterOptions())
```

*Are the coordinates meaningful:* The coordinates are normal as such, but they are in a weird nested list format which fortunately was fixed with some wrangling.

Oooof. That was some serious wrangling!

### Question: 
*2. Which part of the world are the inscriptions from?*
They seem to be mainly from Italy (both mainland and Sicilia)
...
Because it is the standard datum, which makes it easier to plot in whatever way we like. 
...
insc_ch <- st_convex_hull(st_geometry(insc_sf3035))
...
plot(c_buff, border='red', lwd=2, add = TRUE)
plot(insc_ch, add = TRUE)
...
rural_ratio <- nrow(insc_rural) / nrow(i)
```


### Question: 
*4. What is the ratio of urban to rural inscriptions?*
The ratio is approximately 13 / 2 urban to rural
