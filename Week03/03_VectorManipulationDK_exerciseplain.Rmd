...
author: "Adela Sobotkova feat. Jonathan Rystrøm"
...
raw_dat <- data.frame(place = c("Aarhus Domkirke", 
                           "Christianskirken",
                           "Nobelparken"),
...
# Convert
df <- st_as_sf(raw_dat, coords = c("longitude", "latitude"), crs=4326)

# Change CRS
df <- st_transform(df, crs=st_crs(shelters))

# Buffer for shelter
shelter_buffer <- st_buffer(shelters, 500)

# Plot stuff together
plot(shelters['bookbar']); plot(shelter_buffer['bookbar'], add=T); plotRGB(aarhus, stretch='lin', add=T)
```
...
plot(municipalities$geometry);plot(centroids, add=T)
```
...
```{r Ar-box, eval= FALSE}
# YOUR CODE
aarhus_mun <- municipalities %>% 
  filter(NAME_2 == "Århus")

# Checks out
shelters <- read_sf("../data/shelters.json")
plot(aarhus_mun$geometry); plot(shelters, add=T)

# bbox
aarhus_bbox <- st_bbox(aarhus_mun)

# Make grid 
aarhus_grid <- st_make_grid(aarhus_mun, n=1) %>% 
  st_transform(crs(shelters))

plot(municipalities$geometry); plot(aarhus_grid, add=T)

# Orthophoto
aarhus_photo <- brick("../data/Aarhus_1m.TIF")

crs(shelters)

st_crs(aarhus_bbox) == st_crs(aarhus_photo)

plot(aarhus_grid); plot(shelters$geometry, add=T);plotRGB(aarhus_photo, add=T)
```
